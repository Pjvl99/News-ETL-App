...
def all():
    ...
    select = "SELECT * EXCEPT(created_at, number, subcategory, news_site) FROM `trim-heaven-415202.news_prd.news_numbered`"
...
def search():
    ...
    select = f'''WITH cte AS (SELECT * EXCEPT(created_at, number, subcategory, news_site), 
                    row_number() OVER (ORDER BY date DESC) as row_num FROM `trim-heaven-415202.news_prd.news_numbered`  WHERE title LIKE '%{value}%') '''
...
def getcategories():
    df = client.query("SELECT DISTINCT category FROM `trim-heaven-415202.news_prd.news_numbered`").to_dataframe()
    return Response(df.to_json(orient="records"), mimetype="application/json")
...
def category():
    ...
    select = f'''WITH cte AS (SELECT * EXCEPT(created_at, number, news_site, category), row_number() OVER (ORDER BY date DESC) as row_num 
                    FROM `trim-heaven-415202.news_prd.news_numbered` WHERE category='{category}') '''
    df = client.query(select + f"SELECT * EXCEPT(row_num) FROM cte WHERE row_num BETWEEN {page_lower} and {page_upper}").to_dataframe()
    df['date'] = df['date'].astype(str)
    return Response(df.to_json(orient="records"), mimetype="application/json")
